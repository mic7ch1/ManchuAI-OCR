...
def compute_bootstrap_ci(results, n_bootstrap=1000, ci=0.95):
    """Compute bootstrap CI for test results from per-sample Roman metrics."""
    cer_vals = []
    f1_vals = []
    word_acc_vals = []
    inf_time_vals = []
    ...
    for r in results:
        roman_gt = r["roman_gt"]
        roman_pred = r["roman_pred"]

        cer_vals.append(calculate_cer(roman_gt, roman_pred))
        f1_vals.append(calculate_f1_score(roman_gt, roman_pred))
        word_acc_vals.append(1.0 if calculate_word_accuracy(roman_gt, roman_pred) else 0.0)
...
def generate_performance_table(metrics):
    """Generate LaTeX table for Roman transliteration performance comparison."""
    ...
    lines = [
        r"\begin{table}[!htb]",
        r"\TBL{\caption{Roman performance comparison across validation and test datasets.}\label{tab:performance_comparison_roman}}{\centering",
...
def main():
    ...
    write_latex_table(
        TABLES_OUTPUT_DIR,
        "03_performance_comparison_roman.tex",
        table_latex,
