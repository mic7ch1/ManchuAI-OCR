...
from src.evaluation.utils import get_model_path, load_crnn_model, cleanup_gpu, get_checkpoint_path, run_repeated_inference, print_header
...
def _evaluate_best_checkpoint(
    ...
    results = inference_crnn(model, val_dataset, dataset_config, num_samples)
    ...
    results = inference_crnn(model, test_dataset, dataset_config, num_samples)
...
def _evaluate_all_checkpoints(
    ...
    """Evaluate all checkpoints for a CRNN model."""
...
def _evaluate_replications(
    model_name, model_class, dataset_config, test_dataset,
    metrics_dir, predictions_dir, num_iterations, checkpoint_override
):
    ...
    checkpoint_path = get_checkpoint_path(model_name, model_class, checkpoint_override, extension=".pth")
    ...
    checkpoint_step = int(Path(checkpoint_path).stem.split("-")[-1])
    ...
    if len(completed) >= num_iterations:
        ...
        model = load_crnn_model(str(checkpoint_path))
        inference_fn = lambda ds, dc, n: inference_crnn(model, ds, dc, n)
...
def inference_crnn(model, dataset, dataset_config, num_samples):
    """Run CRNN inference on dataset samples."""
    ...
    for sample in tqdm(sampled, desc="Processing samples"):
        ...
        manchu_pred = model.inference(image)
        inference_time = (time.time() - start_time) * 1000

        results.append({
            "manchu_gt": manchu_gt, "roman_gt": roman_gt,
            "manchu_pred": manchu_pred, "roman_pred": "",
